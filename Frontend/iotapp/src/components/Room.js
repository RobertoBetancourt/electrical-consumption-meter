...
class RoomComponent extends React.Component {
  ...
  getType() {
    let icon_type = "blank";
    const room_type = JSON.stringify(this.state.type);
    switch (room_type) {
      case "2":
        icon_type = "bano";
        break;
      ...
      <div>
        ...
        <button className="room-item" onClick={this.handleClick.bind(this)}>
          <div class="terms">
            <img
              className="room-icon"
              src={require("../icons/salas/" + this.getType() + ".png")}
              alt="Icon"
            />
            <div className="room">
              <h2 className="room-name">{this.state.name}</h2>
              {items.map((d) => (
                <h1 className="device-count">{d.devices} Dispositivos</h1>
              ))}
            </div>
          </div>
        </button>
...
ReactDOM.render(<RoomComponent />, document.getElementById("root"));
