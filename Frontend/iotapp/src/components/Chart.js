...
class GenerateChart extends React.PureComponent {
  ...
  async buildDevice1Chart() {
    ...
    if (this.state.view === "Mensual") {
        ...
        serie
          ...
          .on("mouseover", function (event) {
            ...
            tooltip
              .append("text")
              .attr("id", "label")
              .text(Math.round(point.consumption))
              .attr("y", yPosition - 12)
              .attr("x", xPosition);
            tooltip
              .append("line")
      ...
      let response = await fetch(
        `http://localhost:3001/groups/devicesWeekly?week=${this.state.selectedWeek.format(
          "YYYY-MM-DD"
        )}&room=${this.state.room}`
      );
    ...
    if (this.state.view === "Mensual") {
      ...
      try {
        ...
        Object.assign(
          response_2,
          await fetch(
            `http://localhost:3001/groups/optMonthly?month=${this.state.selectedMonth.format(
              "YYYY-MM"
            )}&room=${this.state.room}`
          )
        );
      ...
      if (response_2.status === 200 && response.status === 200) {
        Object.assign(response, await response.json());
        Object.assign(response_2, await response_2.json());
  ...
  buildLegend(data, types) {
      ...
      Object.keys(point).forEach((type) => {
        if (type !== "day") {
          ...
          consumptionByType[type].consumption =
            (consumptionByType[type].consumption || 0) + point[type];
          totalConsumption += point[type];
        }
      });
    });
    Object.keys(consumptionByType).forEach((type) => {
      consumptionByType[type].percentage = Math.round(
        (consumptionByType[type].consumption / totalConsumption) * 100
      );
    });
    ...
    types = types
      .sort((type1, type2) => {
        return (
          consumptionByType[type2].percentage -
          consumptionByType[type1].percentage
        );
      })
      .map((type) => {
  ...
  async buildRoomChart() {
    ...
    try {
      if (this.state.view === "Mensual") {
        response = await fetch(
          `http://localhost:3001/groups/roomsMonthly?month=${this.state.selectedMonth.format(
            "YYYY-MM"
          )}`
        );
        ...
        response = await fetch(
          `http://localhost:3001/groups/roomsWeekly?week=${this.state.selectedWeek.format(
            "YYYY-MM-DD"
          )}`
        );
        data = await response.json();
  ...
  formatWeekSelectLabel = (date, invalidLabel) => {
    ...
    return dateClone && dateClone.isValid()
      ? `Semana del ${dateClone.startOf("week").format("D MMM")}`
      : invalidLabel;
  };
    ...
    return (
      <div className="screen-device">
        ...
        <div className="chart__date">
          <SingleSelect
            className="chart__view-selector"
            label="Selecciona una vista"
            value={this.state.view}
            options={["Mensual", "Semanal"]}
            onChange={(view) => this.handleViewSelection(view)}
          />
          <MuiPickersUtilsProvider
            className="chart__date-selector"
            utils={MomentUtils}
            locale={"es"}
          >
            {this.state.view === "Mensual" ? (
              <DatePicker
                ...
                onChange={(month) => this.handleMonthSelection(month)}
              />
            ) : (
              <DatePicker
                label="Selecciona una semana"
                value={this.state.selectedWeek}
                onChange={(week) => this.handleWeekSelection(week)}
          ...
          <SingleSelect
            className="chart__room-selector"
            label="Selecciona un escenario"
            value={this.state.room}
            options={[{ label: "Todos", value: -1 }].concat(this.state.rooms)}
            onChange={(room) => this.handleRoomSelection(room)}
            style={
              this.state.tab === 0
                ? { visibility: "visible" }
                : { visibility: "hidden" }
...
ReactDOM.render(<GenerateChart />, document.getElementById("root"));
